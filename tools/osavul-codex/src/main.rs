//! osavul-codex — safe-codex plus the osavul spool.
//!
//! Grants `$HOME/.local/share/osavul` (read+write) on top of whatever profile
//! safe-codex selects, through safe-codex's `SAFE_CODEX_EXTRA_ALLOW`
//! extension point (safe-codex >= 0.5.29, urban-sky/tap). Everything else
//! is inherited: aiq account pooling, per-project transcript isolation, and
//! every grant safe-codex ships.
//!
//! Why not a nono profile that `extends: safe-codex`: `extends` resolves the
//! PROMOTED profile by name, while safe-codex runs its BUNDLED template
...
//!
//! `safe-codex` is resolved through PATH. Under an aiq launcher (aiq drops
//! its shim dir from PATH before launching) that is the real wrapper. Invoked
//! directly, whatever `safe-codex` is first on PATH runs, an aiq shim
//! included; the grant is an exported env var, so it survives either route.
//!
//! The spool directory is created if missing: safe-codex skips a grant whose
...
//! Homebrew will not tap it on your behalf. A missing safe-codex is reported
...
const NAME: &str = "osavul-codex";
const WRAPPED: &str = "safe-codex";
const EXTRA_ALLOW: &str = "SAFE_CODEX_EXTRA_ALLOW";
const INSTALL_HINT: &str = "brew install urban-sky/tap/safe-codex";
...
/// untouched). The format is safe-codex's, colon-separated like PATH, so a
...
fn main() {
    ...
    // invocation `osavul-codex selftest` is intercepted; `selftest` followed
    ...
    if let Err(e) = std::fs::create_dir_all(&spool) {
        // Not fatal: safe-codex will report the skipped grant on its own.
        eprintln!(
